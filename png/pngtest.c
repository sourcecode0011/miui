...
#endif
unsigned int rgb888[0x10000];

//============
void rgb565to888()
{
	unsigned int i = 0;
	//	if(!rgb888)rgb888 = (unsigned int *)malloc(0, 0x10000*4);
	do
	{
		rgb888[i] = (8 * i & 0xFF) | (32 * (i & 0x7E0)) | ((i & 0xF100) << 8);
		i++;
	} while (i < 0x10000);
}
	...
	*/
	...
	for (y = 0; y < bitmap->height; ++y) {
		...
		//	pixel_t * pixel = pixel_at(bitmap, x, y);
			unsigned int rgb = rgb888[x*y];
			*row++ = (unsigned char)(rgb>>16);//pixel->red;
			*row++ = (unsigned char)(rgb >> 8);// pixel->green;
			*row++ = (unsigned char)(rgb);// pixel->blue;
		}
	}
